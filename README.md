...
7. **Summer mode (v2-avvikelse).** Inspirerad av Comfortzones inbyggda sommarfunktion, men med rikare data: när dagens högsta är ≥ 15°C, morgondagens högsta ≥ 13°C, utomhus-max senaste 6 h ≥ 12°C, inomhus ≥ default-1.0°C, och det inte är vinter → triggar Weather Anticipation Reduction oavsett tid eller pris. Byggnaden coast:ar på sol- och luftvärme istället för att betala för värme som ändå överskjuts av eftermiddagssolen. **VV-logiken är oförändrad** — endast climate-grenen påverkas av denna gate.
8. **Manuell override av summer mode.** Trösklarna ovan är tunade för sommarhalvåret. I höst- och vårväder kan en mild dag klara samtliga trösklar medan huset — med låg sol, långa nätter och kall mark — ändå vill ha grundvärme. Switchen **Summer Mode** (`switch.<enhetsnamn>_summer_mode`, på som default) stänger då av *enbart* summer-gaten; resten av kaskaden löper vidare oförändrad och climate-grenen avgörs igen av de vanliga väder-, pris- och tidsvillkoren. `binary_sensor.<enhetsnamn>_summer_mode_active` exponerar attributet `manually_disabled` så du ser om `off` beror på vädret eller på dig. Toggeln räknas som user-action och bypassar anti-flap-cooldown — beslutet räknas om vid nästa cykel, inte om 20 minuter.
